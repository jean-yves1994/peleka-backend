...
exports.POST = withHandler(async (request) => {
  rateLimit(`login:${getClientIp(request) || 'unknown'}`, { max: 20, windowMs: 60_000 });
  ...
  await logAudit({ request, actor: { id: user.id, role: user.role }, action: 'auth.login',
    entityType: 'user', entityId: user.id, data: { via: body.email ? 'email' : 'phone' } });
