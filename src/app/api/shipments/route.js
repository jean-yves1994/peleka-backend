...
exports.POST = withHandler(async (request) => {
  ...
  const customer_id =
    user.role === "admin" ? body.customer_id || user.id : user.id;
  ...
  if (isPremier && Number(customer.credit_limit) > 0) {
    const projected =
      Number(customer.outstanding_balance || 0) + Number(quote.total_price);
  ...
  const shipment = await withTransaction(async (client) => {
    ...
    $39,$40,$41,$42,$43,$44,$45,$46
      ...
      [
        ...
        body.delivery_scheduled_at || null,

        false,

        body.parcel_description,
        body.parcel_category || null,
        body.parcel_weight_kg,
...
exports.GET = withHandler(async (request) => {
  ...
  const { rows } = await query(
    `SELECT s.*,
       ...
       ${where
         .replace(/\bcustomer_id\b/g, "s.customer_id")
         .replace(/\brider_id\b/g, "s.rider_id")
  ...
  const {
    ...
    `SELECT COUNT(*)::int AS count FROM shipments s ${where
      .replace(/\bcustomer_id\b/g, "s.customer_id")
      .replace(/\brider_id\b/g, "s.rider_id")
      .replace(/\bstatus\b/g, "s.status")}`,
    params,
