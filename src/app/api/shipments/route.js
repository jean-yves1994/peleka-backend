...
exports.POST = withHandler(async (request) => {
  ...
  const quote = await quoteShipment({
    ...
    pickup_city: body.pickup_city, delivery_city: body.delivery_city,
  ...
  const shipment = await withTransaction(async (client) => {
    const { rows: [s] } = await client.query(
      `INSERT INTO shipments (
         ...
         $1,'awaiting_assignment', $2,$3,$4, $5,$6,$7,
      ...
      [
        ...
        body.pickup_address, body.pickup_city || null, body.pickup_lat, body.pickup_lng, body.pickup_notes || null, body.pickup_scheduled_at || null,
        body.delivery_address, body.delivery_city || null, body.delivery_lat, body.delivery_lng, body.delivery_notes || null, body.delivery_scheduled_at || null,
    ...
    await client.query(
      `INSERT INTO shipment_status_history (shipment_id, from_status, to_status, changed_by, note)
       VALUES ($1, NULL, 'awaiting_assignment', $2, 'Shipment created')`,
      [s.id, user.id]
    );
    if (quote.discount_code) {
      await client.query(`UPDATE discounts SET used_count=used_count+1 WHERE code=$1`, [quote.discount_code]);
    }
    return s;
  });

  await logAudit({ request, actor: user, action: 'shipment.created',
  ...
  try {
    ...
    await Promise.all(admins.rows.map(a => notify({
      userId: a.id, title: 'New shipment awaiting assignment',
      body: `Shipment ${shipment.tracking_number} needs a rider`,
...
exports.GET = withHandler(async (request) => {
  ...
  else if (user.role === 'rider') { params.push(user.id); filters.push(`rider_id=$${params.length}`); }
  ...
  const { rows } = await query(
    `SELECT * FROM shipments ${where} ORDER BY ${orderBy} ${sortDir} LIMIT ${pageSize} OFFSET ${offset}`, params
  );
  const { rows: [{ count }] } = await query(`SELECT COUNT(*)::int AS count FROM shipments ${where}`, params);
